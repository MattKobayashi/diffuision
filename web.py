...
cancel_inference: asyncio.Event | None = None  # one job at a time UI
...
def _create_pipeline(model: str, device: torch.device, *, token: str) -> FluxPipeline:
    ...
    pipe.to(device)
    pipe.enable_attention_slicing()          # optimisation
    return pipe
...
def generate_image(
    ...
    def _cancel_cb(pipe, step_index: int, timestep: int, callback_kwargs: dict):
...
def clear_cache():
    # Free GPU / accelerator memory so a new job can start immediately.
    gc.collect()
...
async def run_generation(
    ...
    token_limit: int = Form(77),
    base_url: str | None = Form(None),
    seed: int = Form(42),
    enhance: bool = Form(False),
):
    global cancel_inference
    cancel_inference = asyncio.Event()
    try:
        ...
        if "cancelled" in str(e).lower():
            ...
            return templates.TemplateResponse(
                "index.html",
                {"request": request, "error": "Generation cancelled by user."},
            )
        raise
    finally:
...
async def cancel_generation():
    if cancel_inference is not None:
        cancel_inference.set()
